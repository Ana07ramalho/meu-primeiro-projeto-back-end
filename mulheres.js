...
const router = express.Router() // configgurando a parte da rota
const cors = require('cors') //pacote cors que permite consumir essa api no front-end

const conectaBancoDeDados = require('./bancoDeDados') //ligando o bando de dados
...
async function corrigeMulher(request, response) {
    try {
        ...
        console.log(erro);
        response.status(500).json({ erro: 'Erro ao corrigir mulher' });
    }
}
...
app.use(router.delete('/mulheres/:id', deletaMulher)) //configurei rota DELETE /mulher/:id
